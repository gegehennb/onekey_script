...
check_new_ver(){
	echo -e "${Info} 请输入 Aria2 版本号，格式如：[ x.x.x ]，获取地址：[ https://github.com/q3aql/aria2-static-builds/releases ]"
	...
	if [[ -z ${aria2_new_ver} ]]; then
		...
		if [[ -z ${aria2_new_ver} ]]; then
			...
			read -e -p "请输入版本号 [ 格式如 x.x.x ] :" aria2_new_ver
...
Download_aria2_conf(){
	mkdir "${file}" && cd "${file}"
	wget --no-check-certificate -N "https://raw.githubusercontent.com/gegehennb/onekey_script/master/service/Aria2/aria2.conf"
	[[ ! -s "aria2.conf" ]] && echo -e "${Error} Aria2 配置文件下载失败 !" && rm -rf "${file}" && exit 1
	wget --no-check-certificate -N "https://raw.githubusercontent.com/gegehennb/onekey_script/master/service/Aria2/dht.dat"
...
Service_aria2(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/gegehennb/onekey_script/master/service/Aria2/aria2_centos -O /etc/init.d/aria2; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/gegehennb/onekey_script/master/service/Aria2/aria2_debian -O /etc/init.d/aria2; then
			echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/aria2
